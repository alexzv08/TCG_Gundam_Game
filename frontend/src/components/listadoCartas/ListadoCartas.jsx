...
const ListadoCartas = ({ cartas, onAddToDeck, onRestToDeck }) => {
    ...
    return (
        <div className="grid grid-cols-5 gap-4 mt-2 listado-cartas">
            {cartas.length > 0 ? (
                cartas.map((carta) => (
                    <Carta key={`${carta.id_coleccion}-${carta.id_carta}`} carta={carta} onAddToDeck={onAddToDeck} onRestToDeck={onRestToDeck} />
                ))
            ) : (
